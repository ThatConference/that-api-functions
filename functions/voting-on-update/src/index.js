...
const dlog = debug('that:api:functions:voting-on-update');
dlog('started');

const defaultVersion = `voting-on-update@${version}`;
...
Sentry.configureScope(scope => {
  scope.setTag('thatApp', 'voting-on-update');
});
  ...
  return firestore.runTransaction(transaction => {
    ...
    return Promise.all([
      ...
      transaction.get(sessionRef).then(doc => {
        ...
        transaction.update(sessionRef, userVote);
      }),
      ...
      transaction.get(eventVoteRef).then(doc => {
        ...
        transaction.update(eventVoteRef, newVote);
      }),
...
// gcloud functions deploy voting-on-update --runtime nodejs10 --trigger-event providers/cloud.firestore/eventTypes/document.update --trigger-resource projects/qa-that/databases/(default)/documents/votes/{vote} --entry-point handler
